...
\begin{parts}
    ...
    \begin{solution}
        ...
        De la tabla (columna $A_s$ \textbf{dB}): $74>71.5\longrightarrow$ Blackman
    \end{solution}
    ...
    \begin{solution}
        ...
        \end{equation}
        De la tabla (columna \textbf{Exact} $\Delta\omega$, ventana Blackman):
        $\Delta\omega=11\pi/L$
        \begin{align}
            0.01\pi=11\pi/L\longrightarrow L=1100\longrightarrow M=1099=L-1
        \end{align}

        Considerando esto, la respuesta a impulso $h[n]$ del filtro FIR queda:
        \begin{equation} 
            h[n]=
            h_{\text{ideal}}[n]\cdot w_\text{Blackman}[n]=
            \frac{\sin(\omega_c)[n-1099/2]}{\pi[n-1099/2]}\cdot w_\text{Blackman}[n]\\
        \end{equation}
        Con $w_\text{Blackman}[n]$ ventana de Blackman de largo $L=1100$
    \end{solution}
